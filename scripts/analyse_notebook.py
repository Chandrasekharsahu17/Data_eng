...
1. Score PQ + HW + SQL with Claude API
...
CHECKLIST_PATH    = “checklistby claude .md”
...
“What does **init**.py do inside a folder? What happens if it’s missing?”,
...
“Difference between **str** and **repr**? When does Python use each automatically?”,
...
“What do **enter** and **exit** do? What Python statement uses them?”
],
“hw”: [
“Build Matrix class that supports +, -, * (element-wise) via operator overloading. Implement **str** to print it nicely.”,
“Create context manager class Timer using **enter** and **exit** that prints how long a code block took.”,
“Create custom exception hierarchy: PipelineError as base, then ExtractionError, TransformationError, LoadError as children.”,
“Implement **len**, **getitem**, **contains** on a custom DataSet class wrapping a list.”,
...
try:
n = int(folder.name.split(”*”)[1])
if (folder / f”day*{n}.ipynb”).exists():
        ...
        if cur:
            for pfx, store in [("pq_", "pq_answers"), ("hw_", "hw_answers"), ("sql_", "sql_answers")]:
                if cur.startswith(pfx):
                    num = int(cur[len(pfx):])
                    # Skip if it's literally just the placeholder comment
                    placeholder = f"# {pfx[:2].upper()}{num} answer"
                    cleaned = src.strip()
                    if cleaned and cleaned != placeholder and len(cleaned) > 15:
...
scores[“pq”][i] = r; scores[“total”] += r[“score”]; scores[“max_total”] += 2
for i, q in enumerate(d.get(“hw”, []), 1):
r = score_one(q, content[“hw_answers”].get(i, “”), “Homework”)
scores[“hw”][i] = r; scores[“total”] += r[“score”]; scores[“max_total”] += 2
for i, q in enumerate(d.get(“sql”, []), 1):
r = score_one(q, content[“sql_answers”].get(i, “”), “SQL”)
scores[“sql”][i] = r; scores[“total”] += r[“score”]; scores[“max_total”] += 2
return scores
